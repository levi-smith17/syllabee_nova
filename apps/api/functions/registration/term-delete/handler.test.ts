...
describe('registration/term-delete', () => {
    ...
    it('returns 404 when term not found', async () => {
        ...
        const result = await handler(makeEvent('t1')) as any
        expect(result.statusCode).toBe(404)
    })

    it('returns 409 when sections reference this term', async () => {
        mockSend
            .mockResolvedValueOnce({ Item: { pk: 'TERM#t1' } })
    ...
    it('deletes term and returns 204', async () => {
        mockSend
            .mockResolvedValueOnce({ Item: { pk: 'TERM#t1' } })
