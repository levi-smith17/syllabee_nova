...
describe('registration/course-delete', () => {
    ...
    it('returns 404 when course not found', async () => {
        ...
        const result = await handler(makeEvent('abc')) as any
        expect(result.statusCode).toBe(404)
    })

    it('returns 409 when sections reference this course', async () => {
        mockSend
            .mockResolvedValueOnce({ Item: { pk: 'COURSE#abc' } })
