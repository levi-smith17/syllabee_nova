...
export const handler = async (
    ...
    try {
        ...
        // Block delete if sections reference this course
        const sectionCheck = await dynamo.send(new ScanCommand({
            TableName: TABLE_NAME,
            FilterExpression: 'begins_with(pk, :prefix) AND sk = :sk AND courseId = :courseId',
            ExpressionAttributeValues: { ':prefix': 'SECTION#', ':sk': 'METADATA', ':courseId': id },
            Limit: 1,
        }))
        if (sectionCheck.Items?.length) {
            return toApiGatewayResponse(conflict('Cannot delete a course that has sections'))
        }
