...
describe('quicklinks/delete', () => {
    ...
    it('returns 404 when item not found', async () => {
        ...
        const result = await handler(makeEvent('ql-1')) as any
        expect(result.statusCode).toBe(404)
    })

    it('deletes quick link and returns 204', async () => {
        mockSend
            .mockResolvedValueOnce({ Items: [{ sk: 'QUICKLINK#000000#ql-1' }] })
