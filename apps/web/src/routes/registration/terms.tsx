...
export default function TermsPage() {
  ...
  const { data: terms = [], isLoading, isError } = useQuery<Term[]>({
    queryKey: ['terms'],
  ...
  const filtered = React.useMemo(() => {
    ...
    return terms.filter(t => {
      if (termLengthFilter !== 'all') return false
  ...
  async function handleUpdate(id: string) {
    ...
    try {
      await apiFetch(`/registration/terms/${id}`, {
        method: 'PUT',
        body: JSON.stringify({ ...editForm, code: editForm.code.toUpperCase(), termLengthId: editForm.termLengthId || undefined }),
      })
      qc.setQueryData<Term[]>(['terms'], prev =>
        (prev ?? []).map(t => t.id === id ? { ...t, ...editForm, code: editForm.code.toUpperCase() } : t)
      )
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 bg-muted border-b px-6 pt-4 pb-3 flex flex-col gap-2">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <Button size="sm" onClick={() => { setShowAdd(true); setEditingId(null) }} className={cn('gap-2 shrink-0 rounded-none', yellowBtn)}>
        ...
        <div className="flex flex-col gap-1.5 md:flex-row md:flex-wrap md:items-center">
          ...
          {termLengths.length > 0 && (
            <div className="flex gap-1.5">
              <button onClick={() => { setTermLengthFilter('all'); setPage(1) }} className={`${prefixChipClass(termLengthFilter === 'all')} grow md:grow-0 h-8`}>All</button>
              {termLengths.map(tl => (
                <button key={tl.id} onClick={() => { setTermLengthFilter(tl.id); setPage(1) }} className={`${prefixChipClass(termLengthFilter === tl.id)} grow md:grow-0 h-8 truncate`}>{tl.label}</button>
              ))}
            </div>
          )}
          <div className="ml-auto">
            <span className="text-xs text-muted-foreground whitespace-nowrap">
              {filtered.length === terms.length ? `${terms.length} term${terms.length !== 1 ? 's' : ''}` : `${filtered.length} of ${terms.length}`}
          ...
          <form onSubmit={handleCreate} className="border p-5 space-y-4 bg-muted/40">
            ...
            {termLengths.length > 0 && (
              <div className="space-y-1.5">
                ...
                <select id="termLengthId" value={form.termLengthId} onChange={fieldSetter('termLengthId')} className="w-full h-9 border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring">
                  <option value="">None</option>
                  {termLengths.map(tl => <option key={tl.id} value={tl.id}>{tl.label} ({tl.weeks} weeks)</option>)}
                </select>
              </div>
            )}
            <div className="flex flex-col-reverse md:flex-row gap-2 justify-between">
              ...
              <Button type="submit" size="sm" className="rounded-none" disabled={saving}>
                {saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Create Term
              </Button>
            </div>
          </form>
        )}
          ...
          <div className="border overflow-hidden">
            {paginated.map(term =>
              editingId === term.id ? (
                <div key={term.id} className="p-4 space-y-3 bg-muted/40 border-b last:border-b-0">
                  ...
                  {termLengths.length > 0 && (
                    <div className="space-y-1">
                      ...
                      <select value={editForm.termLengthId} onChange={editFieldSetter('termLengthId')} className="w-full h-8 border border-input bg-background px-2 text-sm focus:outline-none focus:ring-1 focus:ring-ring">
                        <option value="">None</option>
                        {termLengths.map(tl => <option key={tl.id} value={tl.id}>{tl.label} ({tl.weeks}w)</option>)}
                      </select>
                    </div>
                  )}
                  <div className="flex flex-col-reverse md:flex-row gap-2 justify-between">
                ...
                <div key={term.id} className={cn('flex items-center gap-3 px-4 py-3 border-b last:border-b-0 bg-muted/20', !term.isActive && 'opacity-50')}>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{fmt(term.startDate)} — {fmt(term.endDate)}</p>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <Button size="sm" variant="ghost" className="h-8 px-2 text-xs" onClick={() => handleToggle(term.id, term.isActive)} disabled={togglingId === term.id}>
                      {togglingId === term.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : term.isActive ? 'Deactivate' : 'Activate'}
                    </Button>
                    <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => startEdit(term)}>
                      <Pencil className="h-3.5 w-3.5" />
                    </Button>
                    <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => setDeleteTarget({ id: term.id, name: term.name })} disabled={deletingId === term.id}>
                      {deletingId === term.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}
                    </Button>
                  </div>
                </div>
              )
