...
const prefixChipClass = (active: boolean) =>
  cn(
    'px-3 py-1 text-xs font-medium transition-colors cursor-pointer border',
    active
...
export default function CoursesPage() {
  ...
  async function handleUpdate(id: string) {
    ...
    try {
      await apiFetch(`/registration/courses/${id}`, {
        method: 'PUT',
        body: JSON.stringify({ ...editForm, code: editForm.code.toUpperCase(), creditHours: Number(editForm.creditHours) }),
      })
      qc.setQueryData<Course[]>(['courses'], prev =>
        (prev ?? []).map(c => c.id === id ? { ...c, ...editForm, code: editForm.code.toUpperCase(), creditHours: Number(editForm.creditHours) } : c)
      )
  ...
  function startEdit(course: Course) {
    ...
    setEditForm({ code: course.code, title: course.title, description: course.description ?? '', creditHours: course.creditHours, isInternship: course.isInternship })
  }

  if (isLoading) return <div className="flex-1 flex items-center justify-center text-muted-foreground text-sm">Loading…</div>
  if (isError) return <div className="flex-1 flex items-center justify-center text-destructive text-sm">Failed to load courses</div>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 bg-muted border-b px-6 pt-4 pb-3 flex flex-col gap-1.5">
        ...
        <div className="flex flex-col gap-1.5 md:flex-row md:flex-wrap md:items-center">
          ...
          <div className="flex gap-1.5">
            <button onClick={() => { setPrefixFilter('all'); setPage(1) }} className={`${prefixChipClass(prefixFilter === 'all')} grow md:grow-0 h-8`}>All</button>
            {prefixes.map(p => (
              <button key={p} onClick={() => { setPrefixFilter(p); setPage(1) }} className={`${prefixChipClass(prefixFilter === p)} grow md:grow-0 h-8`}>{p}</button>
            ))}
          </div>
          <div className="ml-auto">
            <span className="text-xs text-muted-foreground whitespace-nowrap">
              {filtered.length === courses.length ? `${courses.length} course${courses.length !== 1 ? 's' : ''}` : `${filtered.length} of ${courses.length}`}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4">
        ...
        {showAdd && (
          <form onSubmit={handleCreate} className="border p-5 space-y-4 bg-muted/40">
            ...
            <label className="flex items-center gap-2 text-xs cursor-pointer select-none">
              <input type="checkbox" checked={form.isInternship} onChange={e => setForm(f => ({ ...f, isInternship: e.target.checked }))} className="h-3.5 w-3.5 accent-yellow-400" />
              Internship-eligible course
            </label>
            <div className="flex flex-col-reverse md:flex-row gap-2 justify-between">
              <Button type="button" variant="ghost" size="sm" className="bg-muted/70 rounded-none" onClick={() => { setShowAdd(false); setForm(EMPTY_FORM) }} disabled={saving}>Cancel</Button>
              <Button type="submit" size="sm" className="rounded-none" disabled={saving}>
                {saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Create Course
              </Button>
            </div>
          </form>
        )}
        ...
        {filtered.length === 0 ? (
          <p className="text-sm text-muted-foreground italic py-6 text-center">No courses match the current filters.</p>
        ) : (
            ...
            {paginated.map(course =>
              editingId === course.id ? (
                <div key={course.id} className="p-4 space-y-3 bg-muted/40 border-b last:border-b-0">
                  ...
                  <label className="flex items-center gap-2 text-xs cursor-pointer select-none">
                    <input type="checkbox" checked={editForm.isInternship} onChange={e => setEditForm(f => ({ ...f, isInternship: e.target.checked }))} className="h-3.5 w-3.5 accent-yellow-400" />
                    Internship-eligible course
                  </label>
                  <div className="flex flex-col-reverse md:flex-row gap-2 justify-between">
                ...
                <div key={course.id} className={cn('flex items-center gap-3 px-4 py-3 border-b last:border-b-0 bg-muted/20', !course.isActive && 'opacity-50')}>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-sm font-semibold">{course.code}</span>
                      {course.isInternship && <span className="text-xs text-orange-600 dark:text-orange-400">Internship</span>}
                      {!course.isActive && <span className="text-xs text-muted-foreground">(inactive)</span>}
                    </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button size="sm" variant="ghost" className="h-8 px-2 text-xs" onClick={() => handleToggle(course.id, course.isActive)} disabled={togglingId === course.id}>
                      {togglingId === course.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : course.isActive ? 'Deactivate' : 'Activate'}
                    </Button>
                    <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => startEdit(course)}>
                      <Pencil className="h-3.5 w-3.5" />
                    </Button>
                    <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => setDeleteTarget({ id: course.id, code: course.code })} disabled={deletingId === course.id}>
